...
export function getVisionArgs(args: unknown): VisionArgs | null {
  const parsed = parseToolArgs(args);
  if (!parsed?.image_url || !parsed?.question) return null;
  return { imageUrl: String(parsed.image_url), question: String(parsed.question) };
}

export function hasVisionArgs(args: unknown): boolean {
  return getVisionArgs(args) !== null;
}
